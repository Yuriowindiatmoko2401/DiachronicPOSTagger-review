...
class Dataset:
    ...
    :param X_word: a matrix of integers, each row corresponds to a list of the indices of the embeddings of each word in a sentence in the document
    :type X_word: numpy.ndarray
    :param X_year: a list of integers, repeats the year of composition for each word in the sentence for each sentence in the document
    :type X_year: numpy.ndarray
    :param Y_label: a matrix of integers, each row corresponds to a list of the label encoded POS tags of each word in a sentence in the document
    :type Y_label: numpy.ndarray
    :return: Wrapper object stored data
    :rtype: object
    :yield: Wrapper object stored data
    :rtype: object
    """
    ...
    def load(path):
        ...
        :param path: a string, the path where processed X_word, X_year, and Y data is stored
        :type path: string
        :return: a Dataset object, contains loaded X_word, X_year, and Y data
        :rtype: object
        """
    ...
    def iter_batches(self):
        ...
        """
        for i in range(0, len(self.X_word) - BATCH_SIZE, BATCH_SIZE): # 100 batch size
            yield (
                i, 
    ...
    def save(self, savepath):
        ...
        :param savepath: a string, location to save data
...
class TemporalLanguageModel:
    """TemporalLanguageModel object for training stuff
    
    :return: object as construction method inside
    :rtype: object
    """

    def add_graph(self, noyear=False, feedforward=False):
        """Creates a graph for the model. Generates placeholders for X_word, X_year, Y_label, and the embedding matrix. Creates year embedding. Details model architecture. Calculates accuracy, log perplexity, and loss. Optimizes network based on loss.
        
        :param noyear: a boolean, indicates whether year information is included as input to the model, defaults to False
        :type noyear: bool, optional
        :param feedforward: a boolean, indicates whether the model is a feedforward neural network or an LSTM, defaults to False
        :type feedforward: bool, optional
        """
        # Creates placeholders for LSTM
        self.X_word = tf.placeholder(tf.int32, [None, MAX_SENT_LENGTH]) # 50
        self.X_year = tf.placeholder(tf.int32, [None])
        self.Y_label = tf.placeholder(tf.int32, [None, MAX_SENT_LENGTH])
        self.embedding_matrix = tf.placeholder(tf.float32, [MAX_THRESHOLD, EMBED_DIM]) # 600000 x 300

        # Looks up embeddings for each word
        X_word = tf.nn.embedding_lookup(self.embedding_matrix, self.X_word)

        # Creates year embedding
        new_years = tf.subtract(self.X_year, START_YEAR) # 1810
        unembedded_year = tf.tile(tf.expand_dims(new_years, axis=1), [1, MAX_SENT_LENGTH])
        self.year_embed_mat = tf.get_variable(name="year_embed_mat", shape=(NUM_YEAR, EMBED_DIM), initializer=tf.contrib.layers.xavier_initializer()) # NUM_YEAR 200 total delta year x 300 embed dim , xavier_initializer for year embed mat default uniform x = sqrt(6. / (in + out)); [-x, x] <tf.Variable 'year_embed_mat:0' shape=(200, 300) dtype=float32_ref>
        ...
        equal = tf.equal(
            tf.cast(
                tf.argmax(self.Y, axis=2), tf.int32 # cast into tf.int32
    ...
    def train(self, session, train_data, embed_data):
        ...
        :param train_data: a Dataset object, includes train X_word, X_year, and Y_label data
        :type train_data: object
        :param embed_data: a matrix of integers, a matrix of the word embeddings where each row corresponds to a unique word
        :type embed_data: numpy.ndarray
        """
        # Sets a seed
        random.seed(2)
    ...
    def calculate_acc(self, corpus_data, embed_data, train):
        ...
        :param corpus_data: a Dataset object, includes X_word, X_year, and Y_label data for the portion of the data set on which accuracy is being evaluated
        :type corpus_data: object
        :param embed_data: a matrix of integers, a matrix of the word embeddings where each row corresponds to a unique word
        :type embed_data: numpy.ndarray
        :param train: a boolean, indicates whether train or test accuracy is being calculated
        :type train: boolean
        """
    ...
    def linear_reduction(self, feedforward):
        ...
        :param feedforward: a boolean, indicates whether the model is a feedforward neural network or an LSTM 
        :type feedforward: boolen
        """
    ...
    def average_perplexity(self, test_data, embed_data, feedforward, loadperplex, yearbucket):
        """For each test data set sentence, calculates the perplexity of the sentence at all years in the data set. 
        Divides the data set into buckets by either year or decade. 
        For each bucket, fits a LOWESS curve where perplexity is a function of year. 
        For all sentences in a bucket, the predicted year of composition is the year corresponding to the minimum of the corresponding perplexity curve. 
        The evaluation metric for each type of model is the average distance across buckets between this predicted year and the bucket's actual middle year.
        
        :param test_data: a Dataset object, includes X_word, X_year, and Y_label data for the test data
        :type test_data: object
        :param embed_data: a matrix of integers, a matrix of the word embeddings where each row corresponds to a unique word
        :type embed_data: numpy.ndarray
        :param feedforward: a boolean, indicates whether the model is a feedforward neural network or an LSTM
        :type feedforward: boolean
        :param loadperplex: a boolean, indicates whether to load previously calculated perplexity values of each sentence at each year or to calculate these values
        :type loadperplex: boolean
        :param yearbucket: a boolean, indicates whether to load use decades or individual years to aggregate
        :type yearbucket: boolean
        """
        # Verifies model type
        model_type = "FF" if feedforward else "LSTM"
    ...
    def perplexity_sample_sentence(self, X_word_array, actual_year, Y_array, embed_data, word_dict):
        """For a single sentence, calculates the perplexity of the sentence at all years in the data set (1810 to 2009). 
        Takes the predicted year of composition for the sentence to be the year with the minimum perplexity. 
        Returns the sentence's words, actual year of composition, predicted year of composition, and the difference between its actual and predicted years of composition. 
        
        :param X_word_array: a matrix of integers, each row is identical and corresponds to the indices of the embeddings of each word in the given sentence. There are 200 identical rows as having a row for each data set year aids perplexity calculations.
        :type X_word_array: numpy.ndarray
        :param actual_year: an integer, the year of composition of the given sentence
        :type actual_year: integer
        :param Y_array: a matrix of integers, each row is identical and corresponds to the label encoded POS tags of each word in the given sentence. There are 200 identical rows as having a row for each data set year aids perplexity calculations.
    ...
    def find_sample_sentences(self, test_data, embed_data):
        ...
        - documentsentence_sorted - a list of strings, the sentences (ordered by increasing error)
        - indices_sorted - a list of integers, the test data set indices of the sentences (ordered by increasing error)
        :rtypes: 
        - dist_year_sorted - list
        - predicted_year_sorted - list
        - actual_year_sorted - list
        - documentsentence_sorted - list
        - indices_sorted - list
        """
    ...
    def find_minimum_sample_sentences(self, test_data, embed_data, dist_year_sorted, predicted_year_sorted, actual_year_sorted, sentence_sorted, indices_sorted):
        ...
        :param embed_data: a matrix of integers, a matrix of the word embeddings where each row corresponds to a unique word
        :type embed_data: numpy.ndarray
        :param dist_year_sorted: a list of integers, the errors for each sentence in increasing order
        :type dist_year_sorted: list
        :param predicted_year_sorted: a list of integers, the predicted years of composition of each sentence (ordered by increasing error)
        :type predicted_year_sorted: list
        :param actual_year_sorted: a list of integers, the actual years of composition of each sentence (ordered by increasing error)
        :type actual_year_sorted: list
        :param sentence_sorted: a list of strings, the sentences (ordered by increasing error)
        :type sentence_sorted: list
        :param indices_sorted: a list of integers, the test data set indices of the sentences (ordered by increasing error)
        :type indices_sorted: list
        """
        # Generates a dictionary that maps word strings to embedding IDs
        word_dict = generate_word_dict()
...
def generate_word_dict():
    ...
    :return: word_dict or a dictionary with string keys and integer values, maps word strings (keys) to actual embeddings through embedding IDs (values)
    :rtype: dictionary
    """
    # Opens Lexicon
